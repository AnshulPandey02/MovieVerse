...
export default function Header() {
  ...
  const blueButton =
    "px-4 py-2 rounded-lg text-white font-medium bg-gradient-to-r from-[#005C7E] to-[#00A8E1] hover:from-[#00739D] hover:to-[#00C2FF] hover:shadow-[0_0_10px_#00A8E1] transition";
  ...
  return (
    <header className="sticky top-0 left-0 w-full z-50 bg-gradient-to-r from-[#0a0a0a]/80 via-[#111]/80 to-[#0a0a0a]/80 backdrop-blur-md border-b border-gray-800 shadow-md">
      <div className="w-full mx-auto flex justify-between items-center p-4 px-6 md:px-10">
        ...
        <Link
          to="/"
          className="text-white text-3xl font-bold tracking-wide flex items-center"
        >
          <img src={Logo} className="h-7 mt-1" style={{ filter: "hue-rotate(195deg)" }} alt="" />
          ovieVerse
        </Link>
        ...
        <nav className="hidden md:flex gap-8 text-gray-300 text-sm md:text-base">
          <NavLink
            to="/"
            className={({ isActive }) =>
              `transition ${isActive ? "text-[#00A8E1]" : "hover:text-[#00A8E1]"}`
            }
          >
            Home
          </NavLink>
      ...
      {menuOpen && (
        <div className="absolute top-0 left-0 w-full h-80 py-10 bg-black/90 backdrop-blur-2xl flex flex-col items-center justify-between text-lg text-gray-300 z-40 transition-all duration-300">
          <NavLink
            ...
            className="hover:text-[#00A8E1] transition"
          >
            Home
          </NavLink>
          <NavLink
